...
def save_detections(boxes, path, image_name, detections):
    f = open(path, "a")
    line = '"'+image_name+'";'
    if len(boxes)>0:
        line+= " ("
        ...
        for box in boxes:
            try:
                conf = detections[aux-1].confidence
            except IndexError:
                conf = 0.5001
            converted_list = [str(int(round(element))) for element in box]
            line+=", ".join(converted_list)
            line+="):"+str('%.4f' % conf)
            if aux <len(boxes):
                line+=", ("
            else:
